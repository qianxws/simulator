...
var JqdeMods = {
	ajax : function(action, verb, ajaxParams) {
		...
		var url = apiPath + '/commMods/ajax?action=' + action + '&verb=' + verb+'&token=' + token;
	...
	ajaxText : function(action, verb, ajaxParams) {
		var apiPath = Config.apiPath.replace(/\/?$/, '');
		var url = apiPath + '/commMods/ajax?action=' + action + '&verb=' + verb+'&token=' + token;
		return new Promise(function(resolve, reject) {
			var data = null;
...
var BootDialog = {
	dialog : function(options){
		var dl = BootstrapDialog.show({
			...
			message : $('<div aria-hidden="true" data-backdrop="static"></div>').load(options.url),
			draggable : true,
			closable : true,
