...
function setDateValue(date_, span_) {
	if (date_.toLowerCase().indexOf("endtime") != -1) {
		var endTime=$dp.$(date_);
		WdatePicker({skin:'default',onpicked:function(){
			$('#' + span_).hide();
			endTime.click();
		},maxDate:'#F{$dp.$D(\''+date_+'\')}'});
	} else if (date_.toLowerCase().indexOf("starttime") != -1) {
		WdatePicker({skin:'default',onpicked:function(){
			$('#' + span_).hide();
		},minDate:'#F{$dp.$D(\''+date_+'\')}'});
	} else {
