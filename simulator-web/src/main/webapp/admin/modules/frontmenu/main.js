	...
	methods : {
		...
		updateMenu : function() {
			...
			if (is_valid){
				...
				JqdeMods.ajax('frontMenuAction', 'editMenu', {'dataItem' : dataItem}).then(function(result) {
		...
		addMenu : function(pid) {
			JqdeBox.dialog({
				title : '添加菜单',
				url : 'modules/frontmenu/edit.html',
				init : function() {
	            	// 用户权限的初始化操作
	                vmEdit.mounted();
				},
				confirm : function() { // 确认事件
					if (vmEdit.valid()) {
						...
						JqdeMods.ajax('frontMenuAction', 'addMenu', vmEdit.ajaxPramas).then(function(result) {
							if (result && result.success){
		...
		editMenu : function(nodeId) {
			...
			JqdeBox.dialog({
				...
				init : function() {
					// 用户权限的初始化操作
					vmEdit.mounted();
					JqdeMods.ajax('frontMenuAction', 'getMenusByid', {'menuid' : nodeId}).then(function(result) {
						if (result !=null){
//							vmEdit.item=result;
		...
		//选择菜单
		chooseMenu : function(frontmenu) {
			this.selectedMenu = frontmenu;
			//选中样式
			$('#dd-list').find('.active.dd2-content').removeClass('active');
			$('#menu_' + frontmenu.menuid + '>.dd2-content').addClass('active');
		},
		//上移
		...
		saveCseq : function(pid) {
			...
			JqdeMods.ajax('frontMenuAction', 'updateCseq', cseq_array);
		},
		...
		deleteMenu : function(id,pid) {
			...
			JqdeBox.confirm('您是否确定删除菜单，以及子菜单！', function(result) {
				if (result){
					JqdeMods.ajax('frontMenuAction', 'deleteMenu', {'menuid' : id}).then(function(result) {
						if (result && result.success){
	...
	$('#form-horizontal').validate({
		...
		rules : {
			name : {
				...
				remote: {//验证菜单是否存在
	                type:"post",
	                url:"commMods/ajax?action=frontMenuAction&verb=isNotExist&token="+token,
	                data : {
	                	menuid : function() {
	                		return $('#menuid').val()
						}
