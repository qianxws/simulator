...
/**故障库模块Action层*/
@Component
@Service(name = "simFaultLibAction")
public class SimFaultLibAction {

	@Autowired
	private SimFaultLibService simFaultLibService;
	@Autowired
	...
	public JSONObject getList(ActionContext cxt) {
		...
		paraMap.put("faultLibId", MyParam.getString(cxt, "faultLibId"));
		paraMap.put("typeId", MyParam.getString(cxt, "typeId"));
		paraMap.put("deviceName", MyParam.getString(cxt, "deviceName"));
		paraMap.put("faultName", MyParam.getString(cxt, "faultName"));
		paraMap.put("faultContent", MyParam.getString(cxt, "faultContent"));

		//分页查询
		PageHelper.startPage(MyParam.getInt(cxt, "page", 1),MyParam.getInt(cxt, "rows", 15));
		Page<SimFaultLib> p=(Page<SimFaultLib>)simFaultLibService.getList(paraMap);
	...
	public JSONObject getDetail(ActionContext cxt) {
		...
		JSONObject json = new JSONObject();
		json.put("model",simFaultLibService.getDetail(paraMap));
		return json;
	}
	...
	public JSONObject insert(ActionContext cxt) {
		JSONObject json = new JSONObject();
		try {
			TSimFaultLib t = JSON.toJavaObject(MyParam.getDataItem(cxt, "dataItem"), TSimFaultLib.class);
			t.setFaultLibId(MyUUID.getUUID());
			t.setCreateTime(new Date());
			t.setCreator(String.valueOf(cxt.getHttpRequest().getAttribute("userId")));
			simFaultLibService.insert(t);

			json.put("success", true);
			json.put("message","添加故障库成功");
		} catch (Exception e) {
			e.printStackTrace();
			json.put("message", ExceptionUtil.getInsertMessage(e, "添加故障库失败"));
			json.put("success", false);
		}
		logService.addLog("添加故障库", cxt, json);
		return json;
	}
	...
	public JSONObject update(ActionContext cxt) {
		...
		try {
			...
			simFaultLibService.update(t);

			json.put("success", true);
			json.put("message","修改故障库成功");
		} catch (Exception e) {
			e.printStackTrace();
			json.put("message", ExceptionUtil.getUpdateMessage(e, "修改故障库失败"));
			json.put("success", false);
		}
		logService.addLog("修改故障库", cxt, json);
		return json;
	}
	...
	public JSONObject delete(ActionContext cxt) {
		...
		try {
			...
			for (int i = 0; i < ids.size(); i++)
				simFaultLibService.delete(ids.getString(i));
			
			json.put("success", true);
			json.put("message","删除故障库成功");
		} catch (Exception ex) {
			ex.printStackTrace();
			json.put("message", ExceptionUtil.getDeleteMessage(ex, "删除故障库失败"));
			json.put("success", false);
		}
		logService.addLog("删除故障库", cxt, json);
		return json;
	}
}
