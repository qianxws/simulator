...
/**产品类型模块Action层*/
@Component
@Service(name = "simTypeAction")
public class SimTypeAction {
	...
	public JSONObject getList(ActionContext cxt) {
		...
		Page<SimType> p=(Page<SimType>)simTypeService.getList(paraMap);
	...
	public JSONObject getDetail(ActionContext cxt) {
		...
		JSONObject json = new JSONObject();
		json.put("model",simTypeService.getDetail(paraMap));
		return json;
	}
	...
	public JSONObject insert(ActionContext cxt) {
		JSONObject json = new JSONObject();
		try {
			TSimType t = JSON.toJavaObject(MyParam.getDataItem(cxt, "dataItem"), TSimType.class);
			t.setTypeId(MyUUID.getUUID());
			t.setCreateTime(new Date());
			t.setCreator(String.valueOf(cxt.getHttpRequest().getAttribute("userId")));
			simTypeService.insert(t);
			
			json.put("success", true);
			json.put("message","添加产品类型成功");
		} catch (Exception e) {
			e.printStackTrace();
			json.put("message", ExceptionUtil.getInsertMessage(e, "添加产品类型失败"));
			json.put("success", false);
		}
		logService.addLog("添加产品类型", cxt, json);
		return json;
	}
...
//			t.setCinemaId("22");
			t.setCreator("admin");
//			t.setSimTypeTimeLong(11);
			simTypeService.update(t);
			
			json.put("success", true);
			json.put("message","修改产品类型成功");
		} catch (Exception e) {
			e.printStackTrace();
			json.put("message", ExceptionUtil.getUpdateMessage(e, "修改产品类型失败"));
			json.put("success", false);
		}
		logService.addLog("修改产品类型", cxt, json);
		return json;
	}
	...
	public JSONObject delete(ActionContext cxt) {
		...
		try {
			...
			for (int i = 0; i < ids.size(); i++) 
				simTypeService.delete(ids.getString(i));
			
			json.put("success", true);
			json.put("message","删除产品类型成功");
		} catch (Exception ex) {
			ex.printStackTrace();
			json.put("message", ExceptionUtil.getDeleteMessage(ex, "删除产品类型失败"));
			json.put("success", false);
		}
		logService.addLog("删除产品类型", cxt, json);
		return json;
	}
}
