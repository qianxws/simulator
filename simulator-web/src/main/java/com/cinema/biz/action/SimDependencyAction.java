...
/**依赖关系模块Action层*/
@Component
@Service(name = "simDependencyAction")
public class SimDependencyAction {

	@Autowired
	private SimDependencyService simDependencyService;
	@Autowired
	...
	public JSONObject getList(ActionContext cxt) {
		...
		Page<SimDependency> p=(Page<SimDependency>)simDependencyService.getList(paraMap);
	...
	public JSONObject getDetail(ActionContext cxt) {
		...
		JSONObject json = new JSONObject();
		json.put("model",simDependencyService.getDetail(paraMap));
		return json;
	}
	...
	public JSONObject insert(ActionContext cxt) {
		JSONObject json = new JSONObject();
		try {
			TSimDependency t = JSON.toJavaObject(MyParam.getDataItem(cxt, "dataItem"), TSimDependency.class);
			t.setDependencyId(MyUUID.getUUID());
			t.setCreateTime(new Date());
			t.setCreator(String.valueOf(cxt.getHttpRequest().getAttribute("userId")));
			simDependencyService.insert(t);

			json.put("success", true);
			json.put("message","添加依赖关系成功");
		} catch (Exception e) {
			e.printStackTrace();
			json.put("message", ExceptionUtil.getInsertMessage(e, "添加依赖关系失败"));
			json.put("success", false);
		}
		logService.addLog("添加依赖关系", cxt, json);
		return json;
	}
...
//			t.setCinemaId("22");
			t.setCreator("admin");
//			t.setSimDependencyTimeLong(11);
			simDependencyService.update(t);

			json.put("success", true);
			json.put("message","修改依赖关系成功");
		} catch (Exception e) {
			e.printStackTrace();
			json.put("message", ExceptionUtil.getUpdateMessage(e, "修改依赖关系失败"));
			json.put("success", false);
		}
		logService.addLog("修改依赖关系", cxt, json);
		return json;
	}
	...
	public JSONObject delete(ActionContext cxt) {
		...
		try {
			...
			for (int i = 0; i < ids.size(); i++)
				simDependencyService.delete(ids.getString(i));
			
			json.put("success", true);
			json.put("message","删除依赖关系成功");
		} catch (Exception ex) {
			ex.printStackTrace();
			json.put("message", ExceptionUtil.getDeleteMessage(ex, "删除依赖关系失败"));
			json.put("success", false);
		}
		logService.addLog("删除依赖关系", cxt, json);
		return json;
	}
}
